...
class GoodsService {
	...
	async imgList() {
		const statement = ` SELECT 
                          JSON_ARRAYAGG(  
                            CONCAT('${APP_URL}:${APP_PORT}', '/goods/', i.file_name)
                          ) swiper 
                        FROM goods_img i`
	...
	async goodsList(id) {
        ...
        SELECT g.id, g.title, g.detail, g.price,
          ...
          (SELECT JSON_ARRAYAGG(CONCAT('${APP_URL}:${APP_PORT}', '/goods/', i.file_name)) swiper 
        FROM goods_img i WHERE i.goods_id = g.id) img
        FROM goods g
      `
			try {
	...
	async countExist(userId, goodsId) {
		const statement =
			'SELECT * FROM users_goods WHERE user_id = ? AND goods_id = ?'
		try {
	...
	async createCount(id, goodsId, count) {
		const statement =
			'INSERT INTO users_goods (user_id, goods_id, count) VALUES (?, ?, ?);'
		try {
	...
	async updateCount(id, goodsId, count) {
		const statement =
			'UPDATE users_goods SET count = ? WHERE user_id = ? AND goods_id = ?;'
		try {
	...
	async removeCount(id, goodsId) {
		const statement =
			'DELETE FROM users_goods WHERE user_id = ? AND goods_id = ?;'
		try {
